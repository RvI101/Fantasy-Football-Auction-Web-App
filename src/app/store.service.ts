...
export class StoreService {
  ...
  constructor(
    ...
    private leagueFactory: LeagueFactoryService
  ) {
    ...
    this.leaguesRef.stateChanges(["child_removed"]).subscribe((action) => {
      console.log(action);
  ...
  getLeagueTeamMap(leagueId: string): Observable<Map<number, string>> {
    return this.db
      .object(`leagues/${leagueId}/teamMap`)
      .valueChanges()
      .pipe(
        take(1),
        map(
          (t: any) =>
            new Map(
              Object.entries(t).map(([k, v]: [string, string]) => [
                Number(k),
                v,
              ])
            )
  ...
  createLeague(name: string): void {
    ...
    const newLeague$: Observable<[League, string]> = zip(
      this.leagueFactory.getNewLeague(),
      this.uid$
    );
    newLeague$.subscribe(([newLeague, uid]: [League, string]) => {
      ...
      this.db.list(`users/${uid}/leagues`).set(id, {
        leagueId: id,
        name,
        status: "bid",
        squadSize: 0,
        uid,
        money: StoreService.MONEY,
      });
    });
  }
  ...
  joinLeague(leagueId: string): Observable<boolean> {
    ...
    return zip(
      this.getLeague(leagueId).pipe(take(1)),
      canJoin$,
      this.uid$
    ).pipe(
      tap(([league, canJoin, uid]: [League, boolean, string]) => {
        if (canJoin) {
          ...
          this.db.list(`users/${uid}/leagues`).set(leagueId, {
            leagueId,
            name,
            status: "bid",
            squadSize: 0,
          ...
          console.log(league);
          this.db
            .list(`users/${uid}/leagues`)
            .update(leagueId, { name: league.name });
        }
      }),
  ...
  isMember(leagueId: string): Observable<boolean> {
    return this.uid$.pipe(
      mergeMap((uid) =>
        this.db
          .list(`users/${uid}/leagues`, (ref) =>
            ref.orderByChild("leagueId").equalTo(leagueId)
          )
          .valueChanges()
      ),
      take(1),
  ...
  getUserDisplayName(userId?: string): Observable<string> {
    if (userId) {
      return this.db
        .object(`users/${userId}/displayName`)
        .valueChanges()
        .pipe(map((dpName) => dpName as string));
    } else {
      return this.uid$.pipe(
        mergeMap((uid) =>
          this.db.object(`users/${uid}/displayName`).valueChanges()
        ),
  ...
  setUserDisplayName(displayName: string): void {
    this.uid$.subscribe((uid) =>
      this.db.object(`users/${uid}/displayName`).set(displayName)
    );
  }
  ...
  getUserStatus(leagueId: string): Observable<any> {
    return this.uid$.pipe(
      mergeMap((uid) =>
        this.db.object(`users/${uid}/leagues/${leagueId}/status`).valueChanges()
      )
    );
  }
  ...
  markTentativeBid(leagueId: string, bid: any): void {
    this.uid$.subscribe((uid) => {
      const key = this.db
        .list(`users/${uid}/leagues/${leagueId}/tentativeBids`)
        .push(bid).key;
      this.db
        .list(`users/${uid}/leagues/${leagueId}/tentativeBids`)
        .update(key, { key });
  ...
  getTentativeBids(leagueId: string): Observable<any[]> {
    return this.uid$.pipe(
      mergeMap((uid) =>
        this.db
          .list<any>(`users/${uid}/leagues/${leagueId}/tentativeBids`)
          .valueChanges()
      )
    );
  }
  ...
  getBidHistory(leagueId: string): Observable<any[]> {
    return this.uid$.pipe(
      mergeMap((uid) =>
        this.db
          .list<any>(`users/${uid}/leagues/${leagueId}/bidHistory`)
          .valueChanges()
      )
    );
  }
  ...
  getSquadSize(leagueId: string): Observable<number> {
    return this.uid$.pipe(
      mergeMap((uid) =>
        this.db
          .object<number>(`users/${uid}/leagues/${leagueId}/squadSize`)
          .valueChanges()
      )
    );
  }
  ...
  async revokeBids(leagueId: string): Promise<void> {
    console.log("Deleting objects...");
    const squad = await this.uid$
      .pipe(
        mergeMap((uid) =>
          this.db
            .list(`users/${uid}/leagues/${leagueId}/resolvedBids`)
            .valueChanges()
            .pipe(
              take(1),
              map((bids: any) => bids.map((b) => b.player.id))
            )
        )
      )
      .toPromise();
    this.uid$.subscribe((uid: string) => {
      this.db
        ...
        .subscribe(
          (b) => {
            if (squad.includes(b.player.id)) {
              return;
            }
            console.log(b.name);
            const id = b.player.id;
            // Remove bids placed by uid on player id
            console.log(`leagues/${leagueId}/players/${id}/bids`);
            // this.db.list(`leagues/${leagueId}/players/${id}/bids`).remove(uid);
            // Clear user bid history
            console.log(`users/${uid}/leagues/${leagueId}/bidHistory`);
            // this.db
            //   .list(`users/${uid}/leagues/${leagueId}/bidHistory`, (ref) =>
            //     ref.orderByChild("player.id").equalTo(id)
            //   )
            //   .remove();
          },
          (err) => {
          ...
          () => {
            // Move status to BID
            console.log(`users/${uid}/leagues/${leagueId}/status`);
            // this.db
            //   .object(`users/${uid}/leagues/${leagueId}/status`)
            //   .set("bid");
  ...
  getUserLeagueDetails(leagueId: string, userId: string): Observable<any> {
    return combineLatest([
      this.db.object(`users/${userId}/leagues/${leagueId}`).valueChanges(),
      this.getUserDisplayName(userId),
    ]).pipe(
      map(([ul, memberName]: [any, string]) => ({
        uid: ul.uid,
        status: ul.status,
        squadSize: ul.squadSize,
  ...
  getResolvedBids(leagueId: string, memberId?: string): Observable<any[]> {
    if (memberId) {
      return this.db
        .list(`users/${memberId}/leagues/${leagueId}/resolvedBids`)
        .valueChanges();
    } else {
      return this.uid$.pipe(
        mergeMap((uid) =>
  ...
  resolveBids(leagueId: string): void {
    this.db
      .list<Player>(`leagues/${leagueId}/players`)
      .valueChanges()
      .pipe(take(1))
      .subscribe((players) => {
        players.forEach((p) => {
          if (p.bids && p.status === "Available") {
            const sortedBids = [...Object.values(p.bids)]
              .map((b) => b as any)
            ...
            winBid.player = p;
            this.db
              .list(`users/${p.owner}/leagues/${leagueId}/resolvedBids`)
              .push(winBid);
    ...
    this.db
      .list(`leagues/${leagueId}/members`)
      .valueChanges()
      .pipe(take(1), mergeAll())
      .subscribe((m) => {
        this.db
          .object(`users/${m}/leagues/${leagueId}`)
          .update({ status: "bid" });
      });
  }
  ...
  updateUserLeagueState(leagueId: string): void {
    ...
    const bids$ = this.uid$.pipe(
      mergeMap((uid) =>
        this.db
          .list<any>(`users/${uid}/leagues/${leagueId}/resolvedBids`)
          .valueChanges()
      ),
      take(1),
      mergeAll()
    ...
    zip(this.uid$, bids$.pipe(count())).subscribe(
      ([uid, c]: [string, number]) => {
        this.db
          .object(`users/${uid}/leagues/${leagueId}`)
          .update({ squadSize: c });
  ...
  getUserMoney(leagueId: string): Observable<number> {
    return this.uid$.pipe(
      mergeMap((uid) =>
        this.db
          .object<number>(`users/${uid}/leagues/${leagueId}/money`)
          .valueChanges()
      )
    );
  }
