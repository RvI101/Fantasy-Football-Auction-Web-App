...
export class LeagueHubComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.route.paramMap
      .pipe(
        ...
        switchMap((params: ParamMap) =>
          this.store.getUserStatus(params.get("id"))
        )
      )
      .subscribe((status) => {
    ...
    this.leagueMembers$ = combineLatest([
      this.league$,
      this.afAuth.user.pipe(map((u: User) => u.uid)),
    ]).pipe(
      takeUntil(this.unsubscribe$),
      tap(([l, uid]: [League, string]) => {
        this.isAdmin = l.admin === uid;
      }),
      switchMap(([l, uid]: [League, string]) =>
        combineLatest(
          l.members.map((m) => this.store.getUserLeagueDetails(l.leagueId, m))
        )
      )
    );

    this.leagueMembers$.subscribe((ulist) => {
      this.memberMap = Object.assign(
        {},
        ...ulist.map((uObj: any) => ({ [uObj.id]: uObj }))
      );
      // console.log(ulist);
      const validStatuses = (
        [...Object.values(this.memberMap)] as any[]
      ).filter((ms) => ms.status !== "done");
      this.allWait =
        validStatuses.length &&
        validStatuses.every((ms) => ms.status === "wait");
    });
    ...
    this.isAahir$ = this.afAuth.user.pipe(
      map((u: User) => u.uid),
      mergeMap((uid: string) => this.store.getUserDisplayName(uid)),
      map((dpname: string) => dpname === "RVI")
    );
  }
  ...
  revokeBids(leagueId: string): void {
    this.store.revokeBids(leagueId);
  }
